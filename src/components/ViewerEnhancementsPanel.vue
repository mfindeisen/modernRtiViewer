    ...
    <div class="flex flex-col gap-3 text-xs text-slate-400">
      <template v-if="lineDrawingMode">
        <p class="text-[10px] text-slate-500 leading-snug -mt-1 flex flex-wrap items-center gap-1.5">
          <ExperimentalBadge />
          Black lines from surface curvature. Lower thresholds pick up finer relief.
        </p>
      ...
      <label class="flex flex-col gap-1">
        ...
        <input
          ...
          class="w-full h-1.5 bg-slate-600 rounded-lg appearance-none cursor-pointer accent-amber-400"
...
function emitNumber(
  event: 'update:diffuseGain' | 'update:unsharpAmount' | 'update:specularExponent' | 'update:specularIntensity' | 'update:ridgeThreshold' | 'update:valleyThreshold' | 'update:lineWidth',
  e: Event,
) {
